...
public class ProductDBContext extends DBContext {
    ...
    public List<Product> getProductsWithPagging(int page, int PAGE_SIZE, boolean sort, String sortby) {
        List<Product> list = new ArrayList<>();
        try {
            String sortOrder = sort ? "asc" : "desc";
    ...
    public List<Product> getProductsWithPaggingFilter(int page, int PAGE_SIZE, boolean sort, String sortby, List<Integer> cateids) {
        List<Product> list = new ArrayList<>();

        String sqlcatt = "";
        if (!cateids.isEmpty() && cateids.size() >= 1) {
            sqlcatt += " where cateID in (" + cateids.get(0);
            for (int i = 1; i < cateids.size(); i++) {
                sqlcatt += " , " + cateids.get(i);
            }
            sqlcatt += ")";
        }
        try {
            String sortOrder = sort ? "asc" : "desc";
            String sql = "select * from Product " + sqlcatt + " order by " + sortby + " " + sortOrder + " "
                    + "offset (?-1)*? rows fetch next ? rows only";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, page);
            stm.setInt(2, PAGE_SIZE);
    ...
    public int getTotalProducts(List<Integer> cateids) {
        String sqlcatt = "";
        if (!cateids.isEmpty() && cateids.size() >= 1) {
            sqlcatt += " where cateID in (" + cateids.get(0);
            for (int i = 1; i < cateids.size(); i++) {
        ...
        try {
            String sql = "select count(id)  from Product " + sqlcatt;
    ...
    public List<Product> getRelatedProduct(int id) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = " SELECT TOP 4 p1.* FROM product p1 WHERE p1.cateId = (SELECT cateID FROM product WHERE id = ?) AND p1.id <> ? ORDER BY NEWID()\n"
                    + "\n"
                    + "";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
