...
public class CheckOutController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Cart> cartPay = new ArrayList<>(carts.values());
        //tinh tong tien
        double totalPrice = 0;
        for (Map.Entry<Integer, Cart> entry : carts.entrySet()) {
            Integer productId = entry.getKey();
        ...
        List<Cart> cartss = this._cartDao.getCartById(a.getUid());
        
        Order order = new Order(a.getUid(), totalPrice, note, shippingId);
//        cartss.removeAll(cartPay);
//        this._cartDao.addToCart(cartss, a.getUid());
        for(Cart cart: cartPay){
            this._cartDao.deleteCartById(a.getUid(),cart.getProduct().getId());
        }
        ...
        List<OrderDetail> orderdetail = new OrderDetailDBcontext().getAllOrderDetailById(orderId);
        request.setAttribute("carts", orderdetail);
